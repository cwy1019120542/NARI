...
def generate_result_list(file_path, centre_company_dict, last_file_path, code_department_dict):
    data_list = get_data_list(file_path, 1, 1, ["利润中心", "余额", "3-4年", "4-5年", "5年以上"])
...
def handle_11(file_dir, last_file_dir, centre_company_dict, code_department_dict):
    file_name = "挂账三年以上其他应付账款情况.xlsx"
    origin_file_dir = os.path.join(file_dir, "origin")
    file_path = os.path.join(origin_file_dir, f"{check_file_dict['other_pay_excel']}.xlsx")
